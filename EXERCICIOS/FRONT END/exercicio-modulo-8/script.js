...
function mostrarContatos(letter) {
  ...
  Array.from(tbody.children).forEach((row) => {
    row.style.display =
      row.getAttribute("data-index") === letter ? "table-row" : "none";
  });
}
...
document.querySelector("form").addEventListener("submit", function (e) {
  ...
  const contatosDuplicados = Array.from(
    document.querySelectorAll("#contatosTbody tr")
  ).map((row) => ({
...
document.getElementById("botaoSearch").addEventListener("click", function () {
  ...
  tRows.forEach((row) => {
    ...
    if (
      nome.includes(novaBusca) ||
      telefone.includes(novaBusca) ||
      email.includes(novaBusca)
    ) {
      row.style.display = "";
    } else {
