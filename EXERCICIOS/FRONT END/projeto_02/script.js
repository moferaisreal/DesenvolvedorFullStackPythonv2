...
const imgRecuperacao =
  '<img src="./images/recuperacao.png" alt="emoji em dúvida"/>';
const imgReprovado =
  '<img src="./images/reprovado.png" alt="emoji desapontado"/>';
...
function addLinha() {
  ...
  if (arrayMaterias.includes(nomeMateria.value)) {
    ...
    linha += `<td>${
      valorNota.value <= 5
        ? imgReprovado
        : valorNota.value > 5 && valorNota.value < 7
        ? imgRecuperacao
        : imgAprovado
    }</td>`;
    linha += "</tr>";
...
function updateMedia() {
  ...
  document.getElementById("emojiFinal").innerHTML =
    mediaFinalOutput >= 7 ? imgCheck : imgCross;
